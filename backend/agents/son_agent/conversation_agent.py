...
"""
    ...
    async def chat(
        ...
        try:
            ...
            logger.info(f"✨ [Son] Self-reflection: {reflection}")
            ...
            if spt_needed and self.spt_agent_v2:
                ...
                try:
                    spt_result = await self.spt_agent_v2.process(
                        session_id=f"아들_{session_id}",  # 에이전트별 고유 세션
                    ...
                    logger.info(f"🧠 [Son] SPT instruction: {spt_instruction}")
            ...
            logger.info(f"✅ [Son] session_id={session_id}, response: '{cleaned_content}'")
    ...
    async def chat_stream(
        ...
        try:
            ...
            logger.info(f"✅ [Son] session_id={session_id}, streamed: '{cleaned_response}'")

        except Exception as e:
            logger.error(f"❌ [Son] Streaming error: {e}", exc_info=True)
            for char in "아버지, 다시 한 번 말씀해주시겠어요?":
                yield char
