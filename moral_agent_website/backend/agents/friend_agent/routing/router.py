...
class ConversationRouter:
    ...
    def route_from_stage1(self, state: Dict[str, Any]) -> str:
        ...
        ✨ 카운터 기반 강제 전환: 1회 재질문 후 자동으로 Stage2로 넘어감 (빠른 전환)

        Stage1Handler가 LLM을 사용하여 "일상 생활에 대한 구체적 설명을 했는가?"를
        ...
        # ⚠️ 주의: 실제 threshold는 conversation_agent.py의 determine_stage()에서 확인 (>= 2)
        # 이 체크는 router에서는 사용되지 않음 (conversation_agent.py가 우선)
